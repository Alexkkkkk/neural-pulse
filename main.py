    ...
    if db_conn:
        async with db_conn.execute("SELECT id FROM users WHERE id = ?", (uid,)) as cursor:
            if not await cursor.fetchone():
                ...
                log_step("DB_ACTION", f"Регистрация: {uid}", C["G"])
    ...
    while True:
        try:
            ...
            if USER_CACHE and db_conn:
                count = 0
                for uid, cache in list(USER_CACHE.items()):
                    d = cache.get("data")
                    if not d: continue
...
async def get_balance(user_id: str):
    ...
    if not db_conn: return JSONResponse({"status": "error"}, 500)
...
async def save_game(data: SaveData):
    ...
    log_step("API_SAVE", f"Кэш обновлен: {uid}", C["P"])
    return {"status": "ok"}
...
async def index():
    ...
    return JSONResponse({"err": "Missing index.html"}, 404)
