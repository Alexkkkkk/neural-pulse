...
async def lifespan(app: FastAPI):
    ...
    logger.info(f"📄 Files: {os.listdir(str(BASE_DIR))}")
...
# ПРАВКА ТУТ: Указываем искать шаблоны ВНУТРИ папки static
...
async def serve_game(request: Request):
    ...
    if not index_path.exists():
        logger.error(f"🚨 index.html НЕ НАЙДЕН в {STATIC_DIR}")
        return {"error": "Файл index.html должен лежать в папке static!"}
...
async def buy_boost(data: dict = Body(...)):
    ...
    with sqlite3.connect(str(DB_PATH)) as conn:
        ...
        cost = lvl * 500
        if balance >= cost:
            new_balance, new_lvl = balance - cost, lvl + 1
