...
# 2. Копируем всё содержимое проекта (включая папку images и index.html)
COPY . .

# 3. Создаем папку для базы данных
RUN mkdir -p /app/data && chmod 777 /app/data

# 4. Создаем серверную прослойку http_wrapper.py
# Она будет отдавать твой index.html и запускать бота
RUN echo 'import os, sys, logging, subprocess, threading, time, uvicorn' > /app/http_wrapper.py && \
    ...
    echo '# Раздача статики (картинок)' >> /app/http_wrapper.py && \
    ...
    echo '    time.sleep(3)' >> /app/http_wrapper.py && \
