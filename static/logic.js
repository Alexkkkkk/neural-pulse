const logic = {
    ...
    async init(userIdFromTg = null, userNameFromTg = null, photoUrlFromTg = null) {
        ...
        const photoUrl = photoUrlFromTg || null;
    ...
    async saveWallet(address) {
        ...
        // Если модалка открыта, обновляем текст адреса
    ...
    async disconnectWallet() {
        if (typeof tonConnectUI !== 'undefined') {
            ...
            ui.openM('wallet'); // Перерисовать модалку
        }
    },
    ...
    async buyUpgrade(type) {
        ...
        if (success) {
            if (typeof ui !== 'undefined') { ui.update(); ui.openM(type === 'profit' ? 'mine' : 'boost'); }
            await this.save();
        }
    },
