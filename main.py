...
async def lifespan(app: FastAPI):
    # СОЗДАНИЕ ПАПКИ И ТАБЛИЦ (СТРОГИЙ ПОРЯДОК)
...
async def get_jackpot():
    try:
        with sqlite3.connect(str(DB_PATH)) as conn:
            ...
            return {"status": "ok", "data": {"amount": res[0] if res else 0}}
    except Exception as e:
...
async def save_game(data: SaveData):
    try:
        with sqlite3.connect(str(DB_PATH)) as conn:
            # Используем REPLACE для автоматического создания или обновления записи
            conn.execute("""
...
# Монтируем статику ПОСЛЕ основных роутов
