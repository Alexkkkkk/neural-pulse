...
// Инициализация базы данных с авто-фиксами
const initDB = async () => {
    try {
        await pool.query(`
        ...
        // Принудительное добавление колонок для старых таблиц
...
bot.start((ctx) => {
    ctx.replyWithHTML(
        ...
        Markup.inlineKeyboard([[Markup.button.webApp("ОТКРЫТЬ ПРИЛОЖЕНИЕ", "https://neural-pulse.bothost.ru")]])
    );
});
