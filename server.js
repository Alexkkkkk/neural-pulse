...
// Указываем серверу на твою папку public
...
const initDB = async () => {
    try {
        ...
        `);
        console.log(`v${VERSION} запущен. Папка public активна.`);
    } catch (err) { console.error("DB Error:", err.message); }
};
initDB();

// Математика: Доход в час (PNL) и Энергия
setInterval(async () => {
    try {
        await pool.query(`
            UPDATE users SET 
...
app.post('/api/bonus', async (req, res) => {
    ...
    res.json({ success: false, msg: "Try again tomorrow!" });
});
