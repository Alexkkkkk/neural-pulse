...
app.get('/api/health', async (req, res) => {
    ...
    <head>
        ...
        <style>
            body { background: #05070a; color: #00f2fe; font-family: 'Courier New', monospace; padding: 20px; display: flex; justify-content: center; }
            .hud { 
                border: 2px solid #00f2fe; padding: 25px; width: 100%; max-width: 500px;
                box-shadow: 0 0 20px rgba(0, 242, 254, 0.15); position: relative;
                background: rgba(0, 242, 254, 0.02);
            }
            .scanline { 
                position: absolute; width: 100%; height: 2px; background: rgba(0, 242, 254, 0.1); 
                top: 0; left: 0; animation: scan 4s linear infinite; 
            }
            @keyframes scan { from { top: 0; } to { top: 100%; } }
            h2 { border-bottom: 1px solid #00f2fe; padding-bottom: 10px; margin-top: 0; text-transform: uppercase; letter-spacing: 2px; }
    ...
    <body>
        <div class="hud">
            ...
            <div class="row">
                <span>CPU_LOAD_NODE</span>
                <span>${load.toFixed(1)}% <div class="bar-bg" style="display:inline-block; vertical-align:middle; margin-left:10px;"><div class="bar-fill"></div></div></span>
            </div>
            <div class="row"><span>MEMORY_RSS</span><span>${(mem.rss / 1024 / 1024).toFixed(1)} MB</span></div>
            <div class="row"><span>MEMORY_HEAP</span><span>${(mem.heapUsed / 1024 / 1024).toFixed(1)} MB</span></div>
            <div class="row"><span>PROCESS_UPTIME</span><span>${(process.uptime() / 60).toFixed(1)} MIN</span></div>
            
            <div class="footer">
                NODE_ID: BOTH_NL4_PULSE | TIME: ${new Date().toLocaleTimeString()}<br>
