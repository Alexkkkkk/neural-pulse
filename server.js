...
app.get('/api/user/:id', async (req, res) => {
    ...
    const ref = req.query.ref; // Если перешел по ссылке
    
    let result = await pool.query('SELECT * FROM users WHERE user_id = $1', [uid]);
    if (result.rows.length === 0) {
        await pool.query('INSERT INTO users (user_id, referred_by) VALUES ($1, $2)', [uid, ref || null]);
        if(ref) await pool.query('UPDATE users SET balance = balance + 5000, ref_count = ref_count + 1 WHERE user_id = $1', [ref]);
...
app.get('/api/admin/monitor', async (req, res) => {
    const result = await pool.query('SELECT user_id, balance, rank, ref_count FROM users ORDER BY balance DESC LIMIT 20');
...
bot.start(async (ctx) => {
    ...
    Markup.inlineKeyboard([[Markup.button.webApp('ЗАПУСТИТЬ ТЕРМИНАЛ', url)]]));
});
