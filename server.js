...
const initDB = async () => {
    try {
        ...
        console.log(`v${VERSION} Engine: Online`);
...
app.post('/api/save', async (req, res) => {
    try {
        const { userId, balance, energy, max_energy, click_lvl, pnl, rank, auto_bot } = req.body;
        await pool.query(`
            UPDATE users SET balance=$2, energy=$3, max_energy=$4, click_lvl=$5, pnl=$6, rank=$7, auto_bot=$8
            WHERE user_id=$1
        `, [String(userId), Number(balance), Math.floor(energy), Math.floor(max_energy), Math.floor(click_lvl), Number(pnl), rank, auto_bot]);
        res.json({ok:true});
    } catch (e) { res.status(500).send(e.message); }
});

app.get('/api/stats', async (req, res) => {
    const r = await pool.query('SELECT user_id, balance, rank FROM users ORDER BY balance DESC LIMIT 15');
    res.json(r.rows);
});
