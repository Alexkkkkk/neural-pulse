...
app.get('/api/health', async (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .ok { color: #33ff66; text-shadow: 0 0 8px #33ff66; }
            .warn { color: #ff3366; text-shadow: 0 0 8px #ff3366; animation: blink 0.8s infinite; }
            @keyframes blink { 50% { opacity: 0.3; } }
            .bar-bg { width: 120px; height: 10px; border: 1px solid #00f2fe; background: #111; display: inline-block; vertical-align: middle; }
            .bar-fill { height: 100%; background: #00f2fe; width: ${Math.min(load, 100)}%; }
    ...
    <body>
        <div class="hud">
            ...
            <div class="row"><span>CPU_LOAD</span><span>${load.toFixed(1)}% <div class="bar-bg"><div class="bar-fill"></div></div></span></div>
            ...
            <div class="footer">ID: BOTH_NL4_PULSE | ${new Date().toLocaleTimeString()}</div>
        </div>
    </body></html>
    `);
});
...
bot.start(async (ctx) => {
    try {
        ...
        const kb = Markup.inlineKeyboard([[Markup.button.webApp("⚡ ВХОД", DOMAIN)]]);
        ctx.reply(`<b>Neural Pulse | Terminal</b>\n\nАгент: <code>${user.username}</code>`, { parse_mode: 'HTML', ...kb });
    } catch (e) { logger.error(`Bot Error: ${e.message}`); }
});

app.get('/api/top', async (req, res) => {
    try {
        res.json(await User.findAll({ limit: 50, order: [['balance', 'DESC']], raw: true }));
    } catch (e) { res.status(500).json([]); }
});

app.listen(PORT, '0.0.0.0', () => logger.system(`ШЛЮЗ АКТИВИРОВАН: Port ${PORT}`));
