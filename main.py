...
API_TOKEN = "8257287930:AAFdsn-kKHnq1yJK6Pbg38iQdGet7S9lOUM" # Твой токен
...
class SaveData(BaseModel):
    ...
    pnl: float = 0
    level: int = 1
    tap_power: int = 1 # Соответствует фронтенду
...
async def get_balance(user_id: str, ref: Optional[str] = None):
    ...
    if row:
        ...
        data["tap_power"] = data.pop("click_lvl") # Маппинг для фронта
        USER_CACHE[uid] = {"data": data, "last_seen": time.time()}
        return {"status": "ok", "data": data}
    
    # Новый пользователь + Реферальная логика
    start_bal = 0.0
    if ref and ref != uid and ref != "":
        try:
            start_bal = 5000.0 # Бонус новичку
            # Начисляем 10,000 пригласившему прямо в БД
...
async def claim_task(payload: TaskClaim):
    ...
    async with db_conn.execute("SELECT 1 FROM completed_tasks WHERE user_id = ? AND task_id = ?", (uid, tid)) as cur:
        if await cur.fetchone(): return {"status": "error", "message": "Already done"}
