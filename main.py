        ...
        try:
            if USER_CACHE and db_conn:
                ...
                for uid, cache in USER_CACHE.items():
                    d = cache.get("data")
                    if not d: continue
                    # Важно: score из фронта пишем в balance базы
...
async def get_balance(user_id: str):
    ...
    if not db_conn:
         return JSONResponse({"status": "error", "msg": "DB not connected"}, 500)
    ...
    res = dict(user)
    res["score"] = res.pop("balance") # Конвертируем balance базы в score для фронта
...
async def save_game(data: SaveData):
    ...
    if uid in USER_CACHE:
        ...
        if data.score > old_score + 100000: # Увеличил лимит до 100к на всякий случай
...
# НОВЫЙ ЭНДПОИНТ ДЛЯ ДЖЕКПОТА
@app.get("/api/jackpot")
async def get_jackpot():
    try:
        # Джекпот — это сумма балансов всех игроков + база 500к
