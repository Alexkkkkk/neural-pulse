...
app.post('/api/save', async (req, res) => {
    try {
        ...
        await pool.query(`
            ...
            WHERE user_id = $1
        `, [String(userId), Number(balance), Math.floor(energy), Math.floor(max_energy || 1000), Math.floor(click_lvl), Number(pnl), rank, auto_bot]);
        res.json({ status: 'ok' });
    } catch (e) { res.status(500).json({ error: e.message }); }
});
...
bot.start((ctx) => {
    const url = `https://${DOMAIN}?ref=${ctx.startPayload || ''}`;
    ctx.replyWithHTML(`<b>🚀 NEURAL PULSE v${VERSION}</b>`, Markup.inlineKeyboard([[Markup.button.webApp('⚡ START', url)]]));
});

app.listen(PORT, () => { console.log(`v${VERSION} Server Online`); bot.launch(); });
