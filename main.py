...
# Исправленный импорт для совместимости с новыми версиями
...
# ВАЖНО: Если Redis на хостинге имеет пароль, формат: redis://:password@host:port
...
async def db_syncer():
    while True:
        ...
        if not USER_CACHE or not db_conn or not redis: continue
        try:
            to_save = []
...
async def lifespan(app: FastAPI):
    ...
    try:
        # Новое подключение через актуальный redis-py
        ...
        async def storm_cmd(m: types.Message):
            if m.from_user.id == ADMIN_ID:
                ...
                await m.answer("⚡️ <b>НЕЙРО-ШТОРМ АКТИВИРОВАН!</b>")
