...
# Настройка CORS для работы Web App
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def get_balance(user_id: str):
    ...
    with sqlite3.connect(str(DB_PATH)) as conn:
        ...
        user_data = dict(row)
        # Расчет оффлайн дохода (1 монета в сек за уровень бота)
        off_time = now - user_data['last_collect']
        profit = 0
        if user_data['bot_lvl'] > 0 and off_time > 60:
            profit = min(off_time * user_data['bot_lvl'], 50000) # Лимит 50к за раз
...
# 1. Монтируем папку static (для картинок и стилей)
